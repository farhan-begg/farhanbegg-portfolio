...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Split text
const header = document.querySelector(".header-1 h1");
const chars = header.textContent.split("").map((c) => {
  const span = document.createElement("span");
  span.classList.add("char");
  span.innerHTML = `<span>${c}</span>`;
  return span;
});
header.innerHTML = "";
chars.forEach((c) => header.appendChild(c));
  ...
  // ----- THREE.JS SETUP -----
  let mainModel, modelSize, currentRotation = 0;
  ...
  window.addEventListener("resize", () => {
    ...
    renderer.setSize(window.innerWidth, window.innerHeight);
    if (mainModel) setupModel(mainModel);
  });

  const tooltipSelectors = [
    {
      trigger: 0.4,
      elements: document.querySelectorAll(".tooltip .line")
    }
  ];
  ...
  ScrollTrigger.create({
    ...
    onUpdate: ({ progress }) => {
      ...
      tooltipSelectors.forEach(({ trigger: t, elements }) => {
        gsap.to(elements, {
          y: progress >= t ? "0%" : "125%",
          ...animOptions
        });
